...
from config import WITHDRAWAL_SCHEDULE, TARGET_ASSET
...
def plot_diagnostics(sim_results: dict, withdrawal_days: list):
    ...
    v_total = sim_results["V_acwi"] + sim_results["V_legacy"]
    ...
    lev_5 = np.percentile(sim_results["Leverage"], 5, axis=0)
    lev_25 = np.percentile(sim_results["Leverage"], 25, axis=0)
    lev_50 = np.median(sim_results["Leverage"], axis=0)
    lev_75 = np.percentile(sim_results["Leverage"], 75, axis=0)
    lev_95 = np.percentile(sim_results["Leverage"], 95, axis=0)
...
def main():
    ...
    print(f"    Target Base: {state['v_acwi_0']:,.2f} CHF")
    print(f"    Legacy Base: {state['v_legacy_0']:,.2f} CHF")
    print(f"[*] Drift Estimates: Target = {params['mu_acwi']:.2%}, Legacy (CHF) = {params['mu_legacy_loc']:.2%}, FX Basket = {params['mu_fx']:.2%}")
    print(f"[*] Correlation Engine: 3x3 Cholesky Matrix Loaded")
    
    # 2. Setup Simulator Engine
    final_date = WITHDRAWAL_SCHEDULE[-1]["date"]
